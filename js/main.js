...
function start() {
    ...
    stand.addEventListener(CLICK, (() => {
        ...
        for (let i = 0; i < DECK.length; i++) {
            if (gameModel.dealer.score <= STAND_DEALER_VALUE || gameModel.dealer.score < gameModel.player.score) {
                let card = DECK.pop()
    ...
    function checkResult(blackjack = false) {
        ...
        // DEALER WIN
        if (gameModel.dealer.score >= gameModel.player.score && gameModel.dealer.score <= MAX_VALUE) {
            dealerWin = true;
        }
